...
//		Quando o for chamado o método buscar leiloes expirados, ele retorna a lista de leilões.
//		O método Mockito.when().thenReturn() devolve a lista de leilões quando o método indicado for chamado.
...
//		Configurando o Mockito para forçar uma Exception para qualquer objeto no método salvar()
		Mockito.when(leilaoDao.salvar(Mockito.any())).thenThrow(RuntimeException.class);

//		Para que os testes sejam efetuados corretamente, necessário capturar a exception nas próximas chamadas.
		try {
			service.finalizarLeiloesExpirados();
